...
public interface CandleRepository extends JpaRepository<Candle, Long> {
    ...
    @Query("select c from Candle c "
        + "where c.coinType = :coinType and c.candleUnit = :candleUnit "
        + "order by c.candleDateTimeKst desc ")
    List<Candle> findAllCandleList(@Param("coinType") CoinType coinType, @Param("candleUnit") CandleUnit candleUnit, Pageable pageable);
}

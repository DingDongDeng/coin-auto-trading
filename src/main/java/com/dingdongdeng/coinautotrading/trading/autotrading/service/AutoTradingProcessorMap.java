...
public class AutoTradingProcessorMap {
  ...
  private final Map<String, AutoTradingProcessor> processorMap =
      new HashMap<>(); // fixme 분산환경을 고려해서 구성이 가능한 내용일까???

  public AutoTradingProcessor get(String processorId) {
    return processorMap.get(processorId);
  }

  public List<AutoTradingProcessor> getList(
      String userId) { // fixme map을 이렇게 순회하지 않도록 수정 필요(db에서 읽던가)

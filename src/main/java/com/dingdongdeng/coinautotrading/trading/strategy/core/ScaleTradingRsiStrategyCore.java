...
public class ScaleTradingRsiStrategyCore implements StrategyCore {
  ...
  public List<TradingTask> makeTradingTask(TradingInfo tradingInfo) {
    ...
    if (!buyTradingResultList.isEmpty()) {
      ...
      if (!profitTradingResultList.isEmpty() || !lossTradingResultList.isEmpty()) {
        ...
        return List.of(TradingTask.builder().isReset(true).build());
      }
    ...
    if (isBuyOrderTiming(rsi, tradingInfo.getCurrentPrice(), tradingResultPack, candles)) {
      if (!isEnoughBalance(
          tradingInfo.getCurrentPrice(), tradingResultPack, tradingInfo.getBalance())) {
      ...
      double currentPrice = tradingInfo.getCurrentPrice();
      double volume =
          buyTradingResultList.isEmpty()
              ? param.getOrderPrice() / currentPrice
              : tradingResultPack.getVolume() * param.getBuyVolumeRate();
      return List.of(
          TradingTask.builder()
              .identifyCode(identifyCode)
              .coinType(coinType)
  ...
  private boolean isEnoughBalance(
      double currentPrice, TradingResultPack tradingResultPack, double balance) {
  ...
  private boolean isBuyOrderTiming(
      double rsi,
      double currentPrice,
      TradingResultPack tradingResultPack,
      ExchangeCandles candles) {
  ...
  private boolean isProfitOrderTiming(
      double currentPrice, double rsi, TradingResultPack tradingResultPack) {
    // 이익 중일때, 이익 한도에 다다르면 익절
    if (currentPrice
        >= tradingResultPack.getAveragePrice() * (1 + param.getProfitLimitPriceRate())) {
      return true;
    }

    // 이익 중일때, RSI 이미 상승했다면 익절
    if (currentPrice > tradingResultPack.getAveragePrice() + param.getOrderPrice() * 0.0005
        && rsi >= param.getProfitRsi()) {
      return true;
    }
    return false;
  }

  private boolean isLossOrderTiming(
      double currentPrice,
      double rsi,
      TradingResultPack tradingResultPack,
      ExchangeCandles candles) {
  ...
  private boolean isTooOld(TradingResult tradingResult) {
    ...
    return ChronoUnit.SECONDS.between(tradingResult.getCreatedAt(), TradingTimeContext.now())
        >= param.getTooOldOrderTimeSeconds();
  }
}

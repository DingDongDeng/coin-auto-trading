...
public class TradingResultPack {
  ...
  public void delete(TradingResult tradingResult) {
    ...
    TradingResult targetTradingResult =
        findTradingResult(tradingResultList, tradingResult.getOrderId());
  ...
  public double getAveragePrice() {
    double buyValueSum =
        buyTradingResultList.stream()
            .mapToDouble(tradingResult -> tradingResult.getPrice() * tradingResult.getVolume())
            .sum();
    double profitValueSum =
        profitTradingResultList.stream()
            .mapToDouble(tradingResult -> tradingResult.getPrice() * tradingResult.getVolume())
            .sum();
    double lossValueSum =
        lossTradingResultList.stream()
            .mapToDouble(tradingResult -> tradingResult.getPrice() * tradingResult.getVolume())
            .sum();

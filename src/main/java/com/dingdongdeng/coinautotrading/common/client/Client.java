...
public abstract class Client {
  ...
  protected <T> T get(
      String path, MultiValueMap<String, String> params, Class<T> clazz, HttpHeaders headers) {
  ...
  protected <T> T get(
      String path, ParameterizedTypeReference<T> parameterizedTypeReference, HttpHeaders headers) {
  ...
  private <T> T responseHandle(Supplier<T> request) {
    try {
      ...
      throw new ApiResponseException(
          e.getStatusCode(), e.getResponseBodyAsString(StandardCharsets.UTF_8), e);
    } catch (Exception e) {

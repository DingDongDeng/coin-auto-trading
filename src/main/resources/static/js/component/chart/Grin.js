// 공식 사이트에서 링크 걸어준 Grin.js : https://gist.github.com/C451/6d8ea8d603e3faadd801b4b3f2e7a57d
...
export default {
  ...
  methods: {
    ...
    draw(ctx) {
      ...
      ctx.strokeStyle = 'black'
      if (this.woo) {
        ctx.arc(c.x, c.y, 42, -Math.PI * 0.1, Math.PI * 1.1)
      }
      let d = this.woo ? 0 : 0.15
      let w = this.woo ? 2 : 0
      ctx.arc(c.x, c.y - (27 - w), 42, Math.PI * 0.9 - d, +Math.PI * .1 + d,
          true)
      if (this.woo) {
        ctx.fill()
      }
      ...
      ctx.lineWidth = 0.25
      if (!this.woo) {
        ctx.arc(c.x, c.y + 15, 25, Math.PI * 0.45, Math.PI * .6)
      }
      ctx.stroke()
      ctx.lineWidth = 0.15
      ctx.beginPath()

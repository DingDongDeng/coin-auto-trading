...
import com.dingdongdeng.coinautotrading.trading.exchange.client.model.UpbitEnum.State;
...
class UpbitClientTest {
  ...
  public void 마켓_정보_조회_테스트() {
    MarketCodeRequest request = MarketCodeRequest.builder().isDetail(true).build();
  ...
  public void 주문_가능_정보_조회_테스트() {
    OrderChanceRequest request =
        OrderChanceRequest.builder().market(MarketType.of(CoinType.ETHEREUM).getCode()).build();
  ...
  public void 주문_리스트_조회_테스트() {
    // 리스트 조회
    OrderInfoListRequest orderInfoListRequest =
        OrderInfoListRequest.builder()
            ...
            .orderBy("desc")
            .build();
    List<OrderResponse> orderInfoListResponse =
        upbitClient.getOrderInfoList(orderInfoListRequest, keyPairId);
  ...
  public void 주문과_조회와_취소_테스트() {
    ...
    OrderInfoRequest orderInfoRequest =
        OrderInfoRequest.builder().uuid(orderResponse.getUuid()).build();
    ...
    OrderCancelRequest orderCancelRequest =
        OrderCancelRequest.builder().uuid(orderResponse.getUuid()).build();
    log.info(
        "result orderCancelResponse: {}", upbitClient.orderCancel(orderCancelRequest, keyPairId));
  }
  ...
  public void 분봉_캔들_조회_테스트() {
    ...
    CandleRequest request =
        CandleRequest.builder().unit(1).market("KRW-ETH").toKst(to).count(2).build();
  ...
  public void 주문_호가_조회_테스트() {
    List<OrderBookResponse> response =
        upbitClient.getOrderBook(
            OrderBookRequest.builder().marketList(List.of(MarketType.KRW_ETH.getCode())).build(),
            keyPairId);
    log.info("result : {}", response);
  }
  ...
  public void 현재가_조회_테스트() {
    List<TickerResponse> response =
        upbitClient.getTicker(
            TickerRequest.builder().marketList(List.of(MarketType.KRW_ETH.getCode())).build(),
            keyPairId);
    log.info("result : {}", response);
  }
}

...
@Slf4j
@SpringBootTest
class UpbitSpotExchangeServiceTest {

    @Autowired
    private ExchangeKeyRepository exchangeKeyRepository;
    @Autowired
    private UpbitSpotExchangeService upbitSpotExchangeService;
    ...
    public void 주문과_조회와_취소_프로세스_테스트() {
        SpotExchangeOrderParam spotExchangeOrderParam = SpotExchangeOrderParam.builder()
        ...
        SpotExchangeOrder spotExchangeOrderInfoResult = upbitSpotExchangeService.getOrderInfo(
            SpotExchangeOrderInfoParam.builder().orderId(exchangeOrderResult.getOrderId()).build(), keyPairId);
        log.info("exchangeOrderInfo result : {}", spotExchangeOrderInfoResult);

        SpotExchangeOrderCancelParam cancelParam = SpotExchangeOrderCancelParam.builder()
            .orderId(exchangeOrderResult.getOrderId())
            .build();
        SpotExchangeOrderCancel cancelResult = upbitSpotExchangeService.orderCancel(cancelParam, keyPairId);
    ...
    public void 거래를_위한_정보_생성_테스트() {
        ...
        log.info("exchangeTradingInfo result : {}", spotExchangeTradingInfo);
    }

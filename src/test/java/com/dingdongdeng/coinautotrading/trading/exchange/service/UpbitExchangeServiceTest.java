...
class UpbitExchangeServiceTest {
  ...
  @Autowired private UpbitExchangeService upbitExchangeService;
  ...
  public void 주문과_조회와_취소_프로세스_테스트() {
    ExchangeOrderParam exchangeOrderParam =
    ...
    ExchangeOrder exchangeOrderInfoResult =
        upbitExchangeService.getOrderInfo(
            ExchangeOrderInfoParam.builder().orderId(exchangeOrderResult.getOrderId()).build(),
            keyPairId);
    log.info("exchangeOrderInfo result : {}", exchangeOrderInfoResult);

    ExchangeOrderCancelParam cancelParam =
        ExchangeOrderCancelParam.builder().orderId(exchangeOrderResult.getOrderId()).build();
  ...
  public void 거래를_위한_정보_생성_테스트() {
    ExchangeTradingInfo exchangeTradingInfo =
        upbitExchangeService.getTradingInformation(

...
class AutoTradingControllerTest {
  ...
  @SpyBean private AutoTradingAggregation autoTradingAggregation;

  @Autowired private ExchangeKeyRepository exchangeKeyRepository;
  @Autowired private UpbitClient upbitClient;
  ...
  public void 사용자_자동매매_리스트_조회_테스트() throws Exception {
    ...
    String strategyIdentifyCode =
        StrategyCode.SCALE_TRADING_RSI.name() + ":" + UUID.randomUUID().toString();
    ...
    Mockito.doReturn(
            List.of(
                ...
                AutoTradingResponse.builder()
                    ...
                    .coinExchangeType(coinExchangeType)
                    .build()))
        .when(autoTradingAggregation)
        .getUserProcessorList(Mockito.any());
                ...
                document(
                    ...
                    responseFields(
                        ...
                        fieldWithPath("body[].title")
                            ...
                            .description("사용자가 등록한 자동매매 이름"),
                        fieldWithPath("body[].processorId")
                            .type(JsonFieldType.STRING)
                            .description("자동매매 프로세스 ID"),
                        fieldWithPath("body[].processDuration")
                            .type(JsonFieldType.NUMBER)
                            .description("프로세스 동작 간격"),
                        fieldWithPath("body[].processStatus")
                            .type(JsonFieldType.STRING)
                            .description("자동매매 프로세스 상태"),
                        fieldWithPath("body[].userId")
                            .type(JsonFieldType.STRING)
                            .description("사용자ID"),
                        fieldWithPath("body[].strategyIdentifyCode")
                            .type(JsonFieldType.STRING)
                            .description("매매 전략 코드 (RSI)"),
                        fieldWithPath("body[].coinType")
                            .type(JsonFieldType.STRING)
                            .description("자동매매 할 코인 종류 (ETHEREUM, DOGE ...)"),
                        fieldWithPath("body[].coinExchangeType")
                            .type(JsonFieldType.STRING)
                            .description("자동거래에 사용할 거래소 종류(upbit)"),
  ...
  public void 자동매매_등록_테스트() throws Exception {
    ...
    AutoTradingRegisterRequest request =
        AutoTradingRegisterRequest.builder()
            .title(title)
                ...
                document(
                    ...
                    requestFields(
                        fieldWithPath("title")
                            .type(JsonFieldType.STRING)
                            .description("사용자가 등록한 자동매매 이름"),
                        fieldWithPath("coinType")
                            .type(JsonFieldType.STRING)
                            .description("자동매매 할 코인 종류 (ETHEREUM, DOGE ...)"),
                        fieldWithPath("coinExchangeType")
                            .type(JsonFieldType.STRING)
                            .description("자동거래에 사용할 거래소 종류(upbit)"),
                        fieldWithPath("tradingTerm")
                            .type(JsonFieldType.STRING)
                            .description("자동매매의 매매타입 (EXTREME_SCALPING, SCALPING,DAY,SWING)"),
                        fieldWithPath("strategyCode")
                            .type(JsonFieldType.STRING)
                            .description("매매 전략 코드 (RSI)"),
                        fieldWithPath("keyPairId")
                            .type(JsonFieldType.STRING)
                            .description("자동매매에 사용할 거래소 키페어 ID")),
                    responseFields(
                        fieldWithPath("body")
        ...
        this.mockMvc
            .perform(
                RestDocumentationRequestBuilders.post(
                        "/autotrading/{autoTradingProcessorId}/start", autoTradingProcessorId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .header("userId", userId))
            .andExpect(status().isOk())
            .andDo(
                document(
                    ...
                    requestHeaders(headerWithName("userId").description("사용자ID")),
                    pathParameters(
                        RequestDocumentation.parameterWithName("autoTradingProcessorId")
                            .description("자동매매 프로세스 ID")),
                    responseFields(
                        fieldWithPath("body")
                            .type(JsonFieldType.OBJECT)
  ...
  public void 자동매매_정지_테스트() throws Exception {
    ...
    Mockito.doReturn(
            AutoTradingResponse.builder()
                ...
                .build())
        .when(autoTradingAggregation)
        .stop(Mockito.any(), Mockito.any());

    MvcResult result =
        this.mockMvc
            .perform(
                RestDocumentationRequestBuilders.post(
                        "/autotrading/{autoTradingProcessorId}/stop", autoTradingProcessorId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .header("userId", userId))
            .andExpect(status().isOk())
            .andDo(
  ...
  public void 자동매매_제거_테스트() throws Exception {
    ...
    Mockito.doReturn(
            AutoTradingResponse.builder()
                ...
                .build())
        .when(autoTradingAggregation)
        .terminate(Mockito.any(), Mockito.any());

    MvcResult result =
        this.mockMvc
            .perform(
                RestDocumentationRequestBuilders.post(
                        "/autotrading/{autoTradingProcessorId}/terminate", autoTradingProcessorId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .header("userId", userId))
            .andExpect(status().isOk())
            .andDo(
                document(
                    "autotrading/terminate",
                    ApiDocumentUtils.getDocumentRequest(),
                    ApiDocumentUtils.getDocumentResponse(),
                    requestHeaders(headerWithName("userId").description("사용자ID")),
                    pathParameters(
